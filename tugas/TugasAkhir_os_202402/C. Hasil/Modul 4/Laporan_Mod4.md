...
1. **System Call `chmod(path, mode)`**  
   System call chmod() ditambahkan untuk memungkinkan proses pengguna mengubah izin akses file secara sederhana, yaitu antara read-only dan read-write. Fungsi ini bekerja dengan memodifikasi bit mode pada struktur inode dari file yang dituju. Dengan implementasi ini, file dapat dibatasi agar tidak bisa ditulisi oleh proses lain (read-only), atau diizinkan untuk dibaca dan ditulis (read-write), tergantung nilai mode yang diberikan.

2. **Driver Device `/dev/random`**  
   Sebuah character device bernama /dev/random ditambahkan ke sistem sebagai antarmuka untuk menghasilkan byte acak saat dibaca oleh program pengguna. Saat file device ini dibuka dan dibaca, driver akan mengembalikan nilai byte acak (misalnya dari fungsi rand() atau generator acak sederhana), yang dapat digunakan untuk berbagai keperluan seperti testing, pembuatan ID unik, atau simulasi. Device ini ditambahkan ke tabel device xv6 dan dihubungkan ke handler fungsi read khusus.
...
```
### 📸 Screenshot:
![hasil chmodtest](./Screenshot/chmodtest_randomtest.jpg)
...
```
### 📸 Screenshot:
![hasil randomtest](./Screenshot/chmodtest_randomtest.jpg)
